...
'''
def Fortinet(comment):
    endPos = len(comment)-1
    stPos = comment.find("Fortinet Result: ")+19
    endPos = comment.find("Update")-1
    result = comment[stPos:endPos]
    return result

def Kaspersky(comment):
    stPos = comment.find("Kaspersky Result: ")+18
    endPos = comment.find("Update", stPos)-1
    result = comment[stPos:endPos]
    return result
'''

def Result(comment, antivirus):
    stPos = comment.find(antivirus) + len(antivirus) + 10
    endPos = comment.find("Update", stPos)-1
    result = comment[stPos:endPos]
    return result
